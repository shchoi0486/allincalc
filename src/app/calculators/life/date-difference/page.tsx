  ...
  const infoSection = {
    calculatorDescription: `
      <div className="space-y-4">
        <p className="text-lg">날짜 차이 계산기는 두 특정 날짜 사이의 간격을 년, 월, 일 단위로 정확하게 계산해주는 강력하고 직관적인 도구입니다.</p>
        ...
        <div className="p-4 bg-gray-100 dark:bg-gray-800 rounded-lg border-l-4 border-blue-500">
          <h3 className="font-semibold text-md mb-2">주요 기능 및 특징:</h3>
    ...
    calculationFormula: `
      <div className="space-y-6">
        <div>
          <h3 className="font-semibold text-xl mb-2 border-b-2 border-gray-200 pb-2">날짜 차이 계산의 기술적 원리</h3>
          <p className="mb-4">이 계산기는 <strong>date-fns 라이브러리</strong>를 사용하여 날짜 계산의 정확성과 효율성을 보장합니다. <code>date-fns</code>는 모던 JavaScript 환경에 최적화된 날짜 유틸리티 라이브러리로, 각 함수가 순수 함수(pure function)로 이루어져 있어 예측 가능한 결과를 제공합니다.</p>
        </div>

        <div>
          <h3 className="font-semibold text-lg mb-2">1. 기간 계산 (년, 월, 일)</h3>
          <p className="mb-2">두 날짜 사이의 전체적인 '기간'을 계산하기 위해 <code>intervalToDuration</code> 함수를 사용합니다.</p>
          <div className="p-4 bg-gray-100 dark:bg-gray-800 rounded-lg">
            <code className="text-sm">const duration = intervalToDuration({ start, end });</code>
          </div>
          <p className="mt-2">이 함수는 시작일(start)과 종료일(end)을 인자로 받아, 두 날짜 사이의 차이를 년(years), 월(months), 일(days) 단위로 나누어 객체 형태로 반환합니다. 예를 들어, 2023년 1월 15일과 2024년 3월 20일 사이의 기간은 { years: 1, months: 2, days: 5 } 와 같이 계산됩니다.</p>
        </div>

        <div>
          <h3 className="font-semibold text-lg mb-2">2. 총 일수 계산</h3>
          <p className="mb-2">전체 기간을 '일' 단위로만 환산하기 위해서는 <code>differenceInDays</code> 함수를 사용합니다.</p>
          <div className="p-4 bg-gray-100 dark:bg-gray-800 rounded-lg">
            <code className="text-sm">let diffDays = differenceInDays(end, start);</code>
          </div>
          <p className="mt-2">이 함수는 두 날짜 사이의 전체 24시간 '일'의 차이를 계산합니다. 시간, 분, 초는 무시하고 오직 날짜만을 기준으로 계산하여 정수 값을 반환합니다.</p>
        </div>

        <div>
          <h3 className="font-semibold text-lg mb-2">3. 초일산입(初日算入) 처리</h3>
          <p className="mb-2">민법 원칙에 따라 기간 계산 시 첫날을 포함하지 않는 것이 일반적이지만, 계약이나 상황에 따라 첫날을 포함해야 할 경우가 있습니다. '초일산입' 옵션이 활성화되면, 계산된 총 일수에 1을 더하여 첫날을 기간에 포함시킵니다.</p>
          <div className="p-4 bg-gray-100 dark:bg-gray-800 rounded-lg">
          ...
          <p className="mt-2">이 간단한 조건문 추가를 통해 법률적, 행정적 요구사항에 맞는 정확한 기간 산정이 가능해집니다.</p>
        </div>
      </div>
    `,
    usefulTips: `
      <div className="space-y-6">
        <div className="p-4 rounded-lg border-l-4 border-green-500 bg-green-50 dark:bg-green-900/20">
          <h3 className="font-semibold text-lg mb-2">💡 '초일산입' 완벽 이해하기</h3>
          <p className="mb-2">'초일산입'은 날짜 계산에서 가장 헷갈리는 부분 중 하나입니다. 쉽게 이해해 보세요.</p>
        ...
        <div className="p-4 rounded-lg border-l-4 border-yellow-500 bg-yellow-50 dark:bg-yellow-900/20">
          <h3 className="font-semibold text-lg mb-2">- 실생활 활용 예시 -</h3>
          <ul className="list-disc list-inside space-y-2">
