...
const JsonPrettifier = () => {
  ...
  const inputSection = (
    <div className="space-y-4">
      <div className="space-y-2">
        <label className="text-sm font-medium">Minified or compact JSON</label>
        <Textarea
          value={input}
      ...
      <Button onClick={prettify} className="w-full">Prettify JSON</Button>
    </div>
  );
  ...
  const resultSection = error ? (
    ...
    <div className="flex items-center justify-center text-muted-foreground h-full">
      Paste JSON and click Prettify to format it
    </div>
  );
