...
const XmlGenerator = () => {
  ...
  const inputSection = (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <label className="text-sm font-medium">Item element name</label>
        <Input value={itemName} onChange={(e) => setItemName(e.target.value)} placeholder="item" />
      </div>
      <div className="space-y-2">
        <label className="text-sm font-medium">Field names (comma separated)</label>
  ...
  const resultSection = result ? (
    <div className="space-y-3">
      <div className="flex gap-2">
        ...
        <Button variant="outline" size="sm" onClick={downloadXML} className="flex-1">Download</Button>
      </div>
      <Textarea readOnly value={result} className="min-h-[300px] font-mono text-xs" />
    </div>
  ) : (
    <div className="flex items-center justify-center text-muted-foreground h-full">
      Configure options and click Generate to create XML data
    </div>
  );
